...
uint32_t u32IAP_PrepareSectors(uint32_t u32StartSector, uint32_t u32EndSector)
{
	...
	else
	{
		...
		__disable_irq();IAP_EXECUTE_CMD(au32Command, au32Result);__enable_irq();
...
**
******************************************************************************/
uint32_t u32IAP_CopyRAMToFlash(uint32_t u32SrcAddr, uint32_t u32DstAddr, uint32_t u32Len)
{
	uint32_t au32Result[5];
	uint32_t au32Command[5];
